...
Checks every certificate and the qualitative behavior of ob(s):
  ...
  ob(s) sanity: raising c_plus increases ob; ob ~ 0 when c_plus = realized.
...
from repairs import (certify_wrappers, repair_transport, repair_selection,
                     repair_presence, repair_random_heads, margin)
...
mp, _ = repair_presence(model, ids, tok_g, tok_c, S, donor, [1, 2])
assert abs(mp - m0) < 1e-3, "self-donor presence patch must be a no-op"
print(f"repairs run: base {m0:+.3f} | transport {mt:+.3f} | "
      f"selection {ms:+.3f} | random {mr:+.3f} | presence(self)=no-op OK")
