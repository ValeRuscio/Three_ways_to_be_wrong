...
bottleneck}.csv for every model and produces in results/aggregate/:
  ...
  T2_ob_separation.csv    ob(s) class medians and AUCs per model
...
def main():
    ...
    for m, df in per_model("obstruction.csv"):
        ...
        rows.append(dict(
            model=m,
            med_presence=pd.Series(g("presence")).median(),
    ...
    if frames:
        t3 = pd.concat(frames)
        pooled = t3.groupby("task")[["black-box", "internal", "sheaf"]].mean()
    ...
    try:
        ...
        if len(t1):
            ax = t1.set_index("model")[
                ["presence", "transport", "selection"]].plot(
        ...
        if frames:
            ...
            order = ["presence", "transport", "selection", "correct"]
            ...
            ax.set_ylabel("ob(s)"); ax.set_title("pooled across models")
